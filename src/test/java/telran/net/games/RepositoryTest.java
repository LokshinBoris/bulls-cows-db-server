...
public class RepositoryTest {
	...
	void createGameGamerMoveAllGameGamersMovesTest() {
	  ...
	  @Test void isGameFinishedTest() {
	  assertFalse(repository.isGameFinished(gameId)); }
	  
	
	  @Order(8)
	  
	  @Test void setIsFinishedTest() { repository.setIsFinished(gameId);
	  assertTrue(repository.isGameFinished(gameId)); }
	  
	  @Order(9)
	  
	  @Test void isWinnerTest() { assertFalse(repository.isWinner(gameId,
	  gamerUsername)); }
	  
	  @Order(10)
	  
	  @Test void setWinnerTest() { repository.setWinner(gameId, gamerUsername);
